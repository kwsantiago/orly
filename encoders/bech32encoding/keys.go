...
func SecretKeyToNsec(sk *secp256k2.SecretKey) (encoded []byte, err error) {
	var b5 []byte
...
func PublicKeyToNpub(pk *secp256k2.PublicKey) (encoded []byte, err error) {
	var bits5 []byte
...
// secret key.
func NsecToSecretKey(encoded []byte) (sk *secp256k2.SecretKey, err error) {
	var b8 []byte
	if b8, err = NsecToBytes(encoded); chk.E(err) {
		return
	}
	sk = secp256k2.SecKeyFromBytes(b8)
	return
}
...
func NsecToBytes(encoded []byte) (sk []byte, err error) {
	...
	sk = sk[:secp256k2.SecKeyBytesLen]
	return
}
...
// public key.
func NpubToPublicKey(encoded []byte) (pk *secp256k2.PublicKey, err error) {
	var b5, b8, hrp []byte
	if hrp, b5, err = bech32.Decode(encoded); chk.E(err) {
...
func HexToPublicKey(pk string) (p *btcec2.PublicKey, err error) {
	if len(pk) != HexKeyLen {
		err = log.E.Err(
...
// encoded public key into a btcec.PublicKey that can be used to verify a
// signature or encode to Bech32.
func HexToSecretKey(sk []byte) (s *btcec2.SecretKey, err error) {
	if len(sk) != HexKeyLen {
		err = log.E.Err(
	...
	if s = secp256k2.SecKeyFromBytes(pb); chk.D(err) {
		return
	}
	return
}
...
func SecretKeyToHex(sk *btcec2.SecretKey) (hexSec []byte) {
