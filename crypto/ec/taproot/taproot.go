...
import (
	...
	"errors"
	"fmt"
	bech33 "orly.dev/crypto/ec/bech32"
	"orly.dev/crypto/ec/chaincfg"
	"orly.dev/utils/chk"
)
...
func decodeSegWitAddress(address []byte) (byte, []byte, error) {
	...
	_, data, bech32version, err := bech33.DecodeGeneric(address)
	if chk.E(err) {
		return 0, nil, err
	}
	...
	regrouped, err := bech33.ConvertBits(data[1:], 5, 8, false)
	if chk.E(err) {
		return 0, nil, err
	}
	...
	if version == 0 && bech32version != bech33.Version0 {
		return 0, nil, fmt.Errorf(
	...
	if version == 1 && bech32version != bech33.VersionM {
		return 0, nil, fmt.Errorf(
...
func encodeSegWitAddress(
	...
	converted, err := bech33.ConvertBits(witnessProgram, 8, 5, true)
	if chk.E(err) {
		return nil, err
	}
	...
	case 0:
		bech, err = bech33.Encode(hrp, combined)

	case 1:
		bech, err = bech33.EncodeM(hrp, combined)
