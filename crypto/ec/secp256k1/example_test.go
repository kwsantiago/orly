...
func Example_encryptDecryptMessage() {
	...
	pubKey, err := secp256k2.ParsePubKey(pubKeyBytes)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	// the recipient.
	ephemeralSecKey, err := secp256k2.GenerateSecretKey()
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	cipherKey := sha256.Sum256(
		secp256k2.GenerateSharedSecret(
			ephemeralSecKey,
			pubKey,
		),
	)
	...
	secKey := secp256k2.SecKeyFromBytes(pkBytes)
	...
	senderPubKey, err := secp256k2.ParsePubKey(senderPubKeyBytes)
	if err != nil {
		fmt.Println(err)
		return
	}
