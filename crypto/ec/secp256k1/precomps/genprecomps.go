...
func serializedBytePoints() []byte {
	...
	doublingPoints := make([]secp256k2.JacobianPoint, curveParams.BitSize)
	var q secp256k2.JacobianPoint
	bigAffineToJacobian(curveParams.Gx, curveParams.Gy, &q)
	for i := 0; i < curveParams.BitSize; i++ {
		// Q = 2*Q.
		doublingPoints[i] = q
		secp256k2.DoubleNonConst(&q, &q)
	}
	...
	for byteNum := 0; byteNum < curveByteSize; byteNum++ {
		...
		// serialize them.
		for i := 0; i < 256; i++ {
			var point secp256k2.JacobianPoint
			for bit := 0; bit < 8; bit++ {
				if i>>uint(bit)&1 == 1 {
					secp256k2.AddNonConst(&point, &windowPoints[bit], &point)
				}
			}
			point.ToAffine()
