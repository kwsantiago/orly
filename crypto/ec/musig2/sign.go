...
func computeSigningNonce(
	...
	*btcec.JacobianPoint, *btcec.ModNScalar, error,
	...
	nonceMsgBuf.Write(schnorr2.SerializePubKey(combinedKey))
...
func Sign(
	...
	challengeMsg.Write(schnorr2.SerializePubKey(nonceKey))
	challengeMsg.Write(schnorr2.SerializePubKey(combinedKey.FinalKey))
...
func verifyPartialSig(
	...
	nonceMsgBuf.Write(combinedNonce[:])
	nonceMsgBuf.Write(schnorr2.SerializePubKey(combinedKey.FinalKey))
	nonceMsgBuf.Write(msg[:])
...
func CombineSigs(
	...
	if opts.combinedKey != nil && opts.tweakAcc != nil {
		...
		//  * e = H(tag=ChallengeHashTag, R || Q || m) mod n
		var challengeMsg bytes.Buffer
		challengeMsg.Write(schnorr2.SerializePubKey(combinedNonce))
		challengeMsg.Write(schnorr2.SerializePubKey(opts.combinedKey))
	...
	return schnorr2.NewSignature(&nonceJ.X, &combinedSig)
}
