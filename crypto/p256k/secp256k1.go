...
func FromSecretBytes(skb []byte) (
	...
	xpkb := make([]byte, schnorr2.PubKeyBytesLen)
	// clen := C.size_t(secp256k1.PubKeyBytesLenCompressed - 1)
	pkb = make([]byte, schnorr2.PubKeyBytesLen)
	var parity Cint
	// ecPub = NewPublicKey()
	pub = NewXPublicKey()
	sec = &Sec{}
...
func Generate() (
	...
	skb = make([]byte, secp256k2.SecKeyBytesLen)
	pkb = make([]byte, schnorr2.PubKeyBytesLen)
	upkb := ToUchar(pkb)
	var parity Cint
	pub = NewXPublicKey()
...
func ECPubFromSchnorrBytes(xkb []byte) (pub *ECPub, err error) {
	if err = AssertLen(xkb, schnorr2.PubKeyBytesLen, "pubkey"); chk.E(err) {
		return
	}
	pub = &ECPub{}
	p := append([]byte{0}, xkb...)
	if C.secp256k1_ec_pubkey_parse(
		...
		secp256k2.PubKeyBytesLenCompressed,
	) != 1 {
...
func PubFromBytes(pk []byte) (pub *Pub, err error) {
	if err = AssertLen(pk, schnorr2.PubKeyBytesLen, "pubkey"); chk.E(err) {
		return
	}
	pub = new(Pub)
...
func (p *Pub) PubB() (b []byte) {
	b = make([]byte, schnorr2.PubKeyBytesLen)
...
func Sign(msg *Uchar, sk *SecKey) (sig []byte, err error) {
	sig = make([]byte, schnorr2.SignatureSize)
...
func Sig(b []byte) (sig *Uchar, err error) {
	if err = AssertLen(b, schnorr2.SignatureSize, "sig"); chk.E(err) {
		return
	}
	sig = ToUchar(b)
	return
...
func NewKeygen() (k *Keygen) {
	k = new(Keygen)
	k.cmprLen = C.size_t(secp256k2.PubKeyBytesLenCompressed)
	k.secBytes = make([]byte, secp256k2.SecKeyBytesLen)
	k.comprPubBytes = make([]byte, secp256k2.PubKeyBytesLenCompressed)
