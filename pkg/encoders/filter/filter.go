...
func (f *F) Marshal(dst []byte) (b []byte) {
	...
	if f.Ids != nil && f.Ids.Len() > 0 {
		first = true
		dst = text2.JSONKey(dst, IDs)
		dst = text2.MarshalHexArray(dst, f.Ids.ToSliceOfBytes())
	}
	if f.Kinds.Len() > 0 {
		if first {
			dst = append(dst, ',')
	...
	if len(f.Search) > 0 {
		...
		dst = text2.JSONKey(dst, Search)
		dst = text2.AppendQuote(dst, f.Search, text2.NostrEscape)
	}
		...
		case inVal:
			...
			case Search[0]:
				...
				case Search[1]:
					...
					var txt []byte
					if txt, r, err = text2.UnmarshalQuoted(r); chk.E(err) {
						return
					}
					f.Search = txt
