...
func (ev *E) MarshalWithWhitespace(dst []byte, on bool) (b []byte) {
	...
	dst = text2.JSONKey(dst, jPubkey)
	if on {
		dst = append(dst, ' ')
	}
	dst = text2.AppendQuote(dst, ev.Pubkey, hex.EncAppend)
	dst = append(dst, ',')
	if on {
		dst = append(dst, '\n', '\t')
	}
	// CreatedAt
	dst = text2.JSONKey(dst, jCreatedAt)
	if on {
		dst = append(dst, ' ')
	}
	...
	// Content
	dst = text2.JSONKey(dst, jContent)
	if on {
		dst = append(dst, ' ')
	}
	dst = text2.AppendQuote(dst, ev.Content, text2.NostrEscape)
	dst = append(dst, ',')
	if on {
		dst = append(dst, '\n', '\t')
	}
	// jSig
	dst = text2.JSONKey(dst, jSig)
	if on {
		dst = append(dst, ' ')
	}
	dst = text2.AppendQuote(dst, ev.Sig, hex.EncAppend)
	if on {
		dst = append(dst, '\n')
	}
	// close parentheses
