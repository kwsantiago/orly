...
func main() {
	...
	if read, err = os.OpenFile(
		"read.jsonl", os.O_RDWR|os.O_CREATE|os.O_TRUNC,
		0600,
	); chk.E(err) {
		return
	}
	defer func() { _ = read.Close() }()
	if unmar, err = os.OpenFile(
		"fail_unmar.jsonl", os.O_RDWR|os.O_CREATE|os.O_TRUNC,
		0600,
	); chk.E(err) {
		return
	}
	defer func() { _ = unmar.Close() }()
	if ids, err = os.OpenFile(
		"fail_ids.jsonl", os.O_RDWR|os.O_CREATE|os.O_TRUNC,
		0600,
	); chk.E(err) {
		return
	}
	defer func() { _ = ids.Close() }()
	if tobin, err = os.OpenFile(
		"fail_tobin.jsonl", os.O_RDWR|os.O_CREATE|os.O_TRUNC,
		0600,
	); chk.E(err) {
		return
	}
	defer func() { _ = tobin.Close() }()
	if frombin, err = os.OpenFile(
		"fail_frombin.jsonl", os.O_RDWR|os.O_CREATE|os.O_TRUNC,
		0600,
	); chk.E(err) {
		return
	}
	defer func() { _ = frombin.Close() }()
	if reser, err = os.OpenFile(
		"fail_reser.jsonl", os.O_RDWR|os.O_CREATE|os.O_TRUNC,
		0600,
	); chk.E(err) {
		return
	}
	...
	for scanner.Scan() {
		...
		if progress%1000 == 0 {
			log.I.F(
				"progress: line %d megabytes %f", progress,
