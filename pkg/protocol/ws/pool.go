...
// WithAuthHandler must be a function that signs the auth event when called. it
// will be called whenever any relay in the pool returns a `CLOSED` message with
// the "auth-required:" prefix, only once for each relay
...
// WithEventMiddleware is a function that will be called with all events
// received. more than one can be passed at a time.
...
// MaxLocks is the maximum number of sync.Mutex locks used in a pool
...
// SubMany opens a subscription with the given filters to multiple relays the
// subscriptions only end when the context is canceled
...
// SubManyNonUnique is like SubMany, but returns duplicate events if they come
			...
			subscribe:
				...
				for {
					...
					case evt, more := <-sub.Events:
						...
						if unique {
							if _, seen := seenAlready.LoadOrStore(
								evt.EventId().String(),
								evt.CreatedAt,
							); seen {
								continue
					...
					case <-ticker.C:
						if eose {
							...
							seenAlready.Range(
								func(id string, value *timestamp.T) bool {
